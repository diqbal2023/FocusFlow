...
async function renderLoadedTasksScreen() {
  render(<TasksScreen />);
  await screen.findByTestId('tasks-screen');
}

describe('Tasks screen temporary UI state', () => {
  it('TC_TASK_UI_01 the screen renders the sample tasks', async () => {
    await renderLoadedTasksScreen();
  ...
  it('TC_TASK_UI_02 selecting Complete advances status Pending to In Progress to Completed', async () => {
    await renderLoadedTasksScreen();
  ...
  it('TC_TASK_UI_03 selecting Delete moves a task to Recently Deleted', async () => {
    await renderLoadedTasksScreen();
    ...
    await screen.findByTestId('deleted-task-card-sample-2');
  ...
  it('TC_TASK_UI_06 restoring from Recently Deleted returns the task to the list', async () => {
    await renderLoadedTasksScreen();
    ...
    fireEvent.press(screen.getByTestId('recently-deleted-button'));
    await screen.findByTestId('task-restore-sample-2');
    ...
    expect(await screen.findByTestId('task-card-sample-2')).toBeTruthy();
    expect(screen.getByText('Clean Room')).toBeTruthy();
  });

  it('TC_TASK_UI_07 permanently deleting from Recently Deleted removes the task', async () => {
    await renderLoadedTasksScreen();
    ...
    fireEvent.press(screen.getByTestId('recently-deleted-button'));
    await screen.findByTestId('task-permanent-delete-sample-2');
  ...
  it('TC_TASK_UI_04 selecting Edit loads the task into the form', async () => {
    await renderLoadedTasksScreen();
  ...
  it('TC_TASK_UI_05 saving a new task adds it to the displayed list', async () => {
    await renderLoadedTasksScreen();
    ...
    expect(await screen.findByText('Write Unit Tests')).toBeTruthy();
  ...
  it('TC_TASK_FORM_01 saving invalid input displays a validation error and does not add the task to the displayed list', async () => {
    await renderLoadedTasksScreen();
